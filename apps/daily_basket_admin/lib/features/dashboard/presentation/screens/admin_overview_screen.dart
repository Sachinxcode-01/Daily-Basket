...
class AdminOverviewScreen extends StatelessWidget {
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                ...
                _buildLivePulseBar(dashboard),
                const SizedBox(height: 20),

                // Revenue hero card
                _buildRevenueHeroCard(dashboard),
                ...
                // Operations grid
                const _SectionHeader(title: 'Live Operations', icon: Icons.bolt_rounded),
                const SizedBox(height: 12),
                _buildOpsGrid(),
                const SizedBox(height: 24),

                // Recent order timeline
                const _SectionHeader(title: 'Order Activity Feed', icon: Icons.timeline_rounded),
                const SizedBox(height: 12),
                _buildOrderFeed(),
                const SizedBox(height: 24),

                // Rider fleet summary
                const _SectionHeader(title: 'Fleet Status', icon: Icons.two_wheeler_rounded),
                const SizedBox(height: 12),
                _buildFleetStatus(),
  ...
  Widget _buildQuickStats(AdminDashboardProvider dashboard) {
    return Row(
      children: [
        Expanded(child: _QuickStatChip(icon: Icons.pending_actions_rounded, label: 'Pending', value: '${dashboard.pendingOrders}', color: const Color(0xFFF59E0B))),
        const SizedBox(width: 8),
        const Expanded(child: _QuickStatChip(icon: Icons.inventory_2_outlined, label: 'Low Stock', value: '7', color: Color(0xFFEF4444))),
        const SizedBox(width: 8),
        Expanded(child: _QuickStatChip(icon: Icons.two_wheeler_rounded, label: 'Riders', value: '${dashboard.activeRiders}', color: const Color(0xFF3B82F6))),
        const SizedBox(width: 8),
        const Expanded(child: _QuickStatChip(icon: Icons.star_half_rounded, label: 'Rating', value: '4.8', color: Color(0xFF8B5CF6))),
      ],
    );
  }

  Widget _buildOpsGrid() {
    const ops = [
      {'label': 'Packing Queue', 'count': '8', 'icon': Icons.inventory_rounded, 'color': Color(0xFF0F766E)},
      {'label': 'Out for Delivery', 'count': '14', 'icon': Icons.local_shipping_rounded, 'color': Color(0xFF3B82F6)},
      {'label': 'Riders Online', 'count': '18', 'icon': Icons.two_wheeler_rounded, 'color': Color(0xFF10B981)},
      {'label': 'Support Tickets', 'count': '3', 'icon': Icons.headset_mic_rounded, 'color': Color(0xFFF59E0B)},
    ];
    ...
    return Container(
      ...
      child: Column(
        children: feeds.asMap().entries.map((entry) {
          ...
          final isLast = entry.key == feeds.length - 1;
          return Column(
            children: [
              ListTile(
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        const Spacer(),
        const Text('View All', style: TextStyle(color: Color(0xFF2DD4BF), fontSize: 12, fontWeight: FontWeight.w600)),
        const Icon(Icons.chevron_right_rounded, color: Color(0xFF2DD4BF), size: 16),
      ],
    );
  }
}
