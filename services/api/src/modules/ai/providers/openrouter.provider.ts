...
export class OpenRouterProvider implements AIProvider {
  ...
  async generateResponse(
    ...
    if (!this.apiKey) {
      throw new Error('OPENROUTER_API_KEY is missing or unconfigured');
    }
    ...
    return RetryManager.executeWithRetry(async () => {
      ...
      const response = await fetch(
        'https://openrouter.ai/api/v1/chat/completions',
        {
          ...
          headers: {
            ...
            'HTTP-Referer': 'https://dailybasket.com',
            'X-Title': 'Daily Basket Support AI',
      ...
      return {
        content: choice?.content || 'Hello! How can I assist with your order?',
        providerUsed: this.getProviderName(),
        tokensUsed: data.usage?.total_tokens || 0,
        latencyMs,
      };
